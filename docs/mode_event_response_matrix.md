...
| any | `arm_night` | accepted command/event | set mode `night`, clear pending, reset risk | none | `System Event` |
...
| `night` / `away` | `door_open` | `door_locked=false` and gate passed | start entry delay (`entry_pending=true`) | `buzzer_warn` | `System Event` |
| `night` / `away` | `entry_timeout` | timeout while armed | escalate to `level=alert`, `score=100` | `buzzer_alert` | `Security Alert` |
| `night` / `away` | `window_open` | armed | raise risk by rule score | `buzzer_warn` or `buzzer_alert` | `System Event` or `Security Alert` |
| `night` / `away` | `motion` / `chokepoint` | armed | raise risk by zone/correlation | `buzzer_warn` or `buzzer_alert` | `System Event` or `Security Alert` |
| `night` / `away` | `vib_spike` | armed | raise risk with correlation bonus | `buzzer_warn` or `buzzer_alert` | `System Event` or `Security Alert` |
| `night` / `away` | `door_tamper` | armed | fast raise risk, may clear pending | `buzzer_alert` | `Security Alert` |
