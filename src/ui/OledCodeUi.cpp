...
void OledCodeUi::update(uint32_t nowMs) {
  if (!disp_) return;
  if (showing_result_ && result_until_ms_ != 0 && nowMs >= result_until_ms_) {
    showing_result_ = false;
    result_until_ms_ = 0;
    dirty_ = true;
    render_();
  ...
  int secLeft = -1;
  bool urgent = false;
  if (countdownActive_ && countdownDeadlineMs_ != 0 && nowMs <= countdownDeadlineMs_) {
    secLeft = (int)((countdownDeadlineMs_ - nowMs + 999) / 1000);
...
void OledCodeUi::render_() {
  ...
  if (countdownActive_ && countdownDeadlineMs_ != 0 && nowMs <= countdownDeadlineMs_) {
    const uint32_t secLeft = (countdownDeadlineMs_ - nowMs + 999) / 1000;
    disp_->print(" ");
    disp_->print(secLeft);
    disp_->print("s");
