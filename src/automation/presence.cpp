...
void onDoorUltrasonic(uint32_t nowMs) {
  ...
  if (entryStage_ == EntryStage::unlocked_wait_ultrasonic && nowMs <= entryDeadlineMs_) {
...
void onDoorOpen(uint32_t nowMs) {
  if (exitStage_ == ExitStage::saw_ultrasonic && nowMs <= exitDeadlineMs_) {
...
void onDoorClose(uint32_t nowMs) {
  if (exitStage_ == ExitStage::saw_door_open && nowMs <= exitDeadlineMs_) {
...
void onPirDetected(uint32_t nowMs) {
  ...
  if (entryStage_ == EntryStage::ultrasonic_wait_pir && nowMs <= entryDeadlineMs_) {
  ...
  if (state_ == State::away && lastAwayAtMs_ != 0 && (nowMs - lastAwayAtMs_) <= cfg_.away_revert_pir_ms) {
    setState(State::home, nowMs);
    resetExit();
  }
}
...
void tick(uint32_t nowMs) {
  if (entryStage_ != EntryStage::idle && nowMs > entryDeadlineMs_) {
    resetEntry();
  }

  if (exitStage_ != ExitStage::idle && nowMs > exitDeadlineMs_) {
    if (exitStage_ == ExitStage::door_closed_wait_no_pir) {
