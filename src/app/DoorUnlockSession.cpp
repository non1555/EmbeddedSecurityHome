...
void DoorUnlockSession::update(uint32_t nowMs,
  ...
  if (!sawOpen_) {
    ...
    if (timeLeftMs <= cfg.door_unlock_warn_before_ms &&
        (nextWarnMs_ == 0 || nowMs >= nextWarnMs_)) {
      buzzer.warn();
  ...
  if (doorOpen && openWarnAtMs_ != 0 && nowMs >= openWarnAtMs_) {
    holdWarnActive_ = true;
    if (!holdWarnSilenced_ && (nextWarnMs_ == 0 || nowMs >= nextWarnMs_)) {
      buzzer.warn();
      nextWarnMs_ = nowMs + cfg.door_warn_retrigger_ms;
    }
  }
